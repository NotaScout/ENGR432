...
/*
TODO BLOCK
Possibly set busy flag check to interrupt?
...
// Commands
// This function takes eight individual bits of a binary number and converts it to a singular hex equivalent
uint8_t bit2hex (uint8_t A, uint8_t B, uint8_t C, uint8_t D, uint8_t E, uint8_t F, uint8_t G, uint8_t H) {
 ...
 * or Line 2, character 2 (hex addres 0xC0)
 *******************************/
void place_lcd_cursor(int lineno) {
	if (lineno == 1) {
		LCD_sendInstr(bit2hex(1, 0, 0, 0, 0, 0, 0, 0)); // 1000 0000 for first line
	} else {
		LCD_sendInstr(bit2hex(1, 1, 0, 0, 0, 0, 0, 0)); // 1100 0000 for second line
	}
}
