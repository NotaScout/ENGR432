...
unsigned int global_counter = 500;

unsigned int global_counter_decrement = 500;

 

unsigned int rr_count_on_off = 0;
...
__NO_RETURN static void task_4(void *argument) {
  ...
  for (;;) {
    ...
    LCD_place_cursor(0xC8);

    LCD_send_intString(global_counter_decrement);

    osSemaphoreRelease(lcdSemaphore);

           global_counter_decrement = global_counter_decrement - 1;
...
__NO_RETURN static void thrBUT(void *argument) {
  ...
  for (;;) {
    ...
    if ( (Buttons_GetState() & (button_msk)) & rr_count_on_off ) { // round robin is going

      osThreadFlagsSet(task_1_ID, 0x0001U);

    } else if ( (Buttons_GetState() & (button_msk)) & !rr_count_on_off ) { // round robin has stopped
